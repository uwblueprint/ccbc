const data = [
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: true,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: 1636226732806,
    books: [
      {
        titlePrefix: "A",
        title: "Testing Author Names Overflow",
        seriesOrder: null,
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null,
          },
          {
            fullName: "Second Author",
            displayName: null,
          },
          {
            fullName: "Third Author",
            displayName: null,
          },
          {
            fullName: "Fourth Author",
            displayName: null,
          },
          {
            fullName: "Fifth Author",
            displayName: null,
          },
          {
            fullName: "Sixth Author",
            displayName: null,
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: null,
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: true,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: 1636226732806,
    books: [
      {
        titlePrefix: "A",
        title: "Testing No Overflow Authors",
        seriesOrder: null,
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null,
          },
          {
            fullName: "Second Author",
            displayName: null,
          },
          {
            fullName: "Third Author",
            displayName: null,
          },
          {
            fullName: "Fourth Author",
            displayName: null,
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: null,
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: true,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: 1636226732806,
    books: [
      {
        titlePrefix: "A",
        title: "Bloom",
        seriesOrder: "Book 1",
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null,
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: "Testing Series Title",
      },
      {
        title: "Hatch",
        seriesOrder: "Book 2",
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null,
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: "Testing Series Title",
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: true,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: 1636226732806,
    books: [
      {
        titlePrefix: "A",
        title: "Testing One Book",
        seriesOrder: null,
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null,
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: null,
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: true,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: 1636226732806,
    books: [
      {
        titlePrefix: "A",
        title: "Testing Author Display Name",
        seriesOrder: null,
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: "Display Name"
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: null,
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
  {
    body:
      "In Schwartz and Gagnon's whimsical picture book, young Samuel lives a quiet life with their parents and a goldfish, Bubble, but they long for adventure. Samuel's quiet home life stands in stark contrast to the kooky sounds that burst out of their babysitter Chloe's home, where there is funky music, dog barks, and enough funny noises to make your head spin. Chloe works as a costume designer, and young readers will be delighted by the variety of outfits, including Victorian gowns and yellow bird suits, seen amongst her rows of costumes. After Chloe shows Samuel a new dazzling dress that she had been working on, Samuel reveals to Chloe that their real name is Simone, but they haven't figured out how to tell their parents yet. This delightful story encourages children to defy traditional gender norms and explore their identity and serves as a wonderful resource for parents and teachers who want to start a conversation with young children about gender identity. Chloe's unwavering support and acceptance of Simone's identity is heartwarming, and one will end the text wishing that they could dance around Chloe's house to music with her and Simone. Readers will also surely be delighted by Chloe's furry companion, aptly named Piano for her black and white spots, who delivers a set of four puppies that Simone names Hamlet, Zeus, Neptune, and Little Miss. Despite their pleading, Simone's parents at first refuse to adopt any of the boisterous new puppies, but after a considerate consultation with Chloe, Simone's parents provide their",
    coverImages: ["thumbnail1.com", "thumnail2.com"],
    byline:
      "Kayla O'Brien is working towards her Master's in English Literature at Queen's University.",
    featured: false,
    createdBy: 1,
    updatedAt: 1636226732806,
    publishedAt: null,
    books: [
      {
        titlePrefix: "A",
        title: "Testing Other Fields",
        seriesOrder: null,
        illustrator: [],
        translator: [],
        formats: [
          {
            format: "pb",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
          {
            format: "hc",
            price: "$50.00",
            isbn: "978-1-44-345031-7",
          },
        ],
        minAge: 12,
        maxAge: 18,
        authors: [
          {
            fullName: "First Author",
            displayName: null
          },
        ],
        publishers: [{ fullName: "HarperCollins Canada", publishYear: 2020 }],
        seriesName: null,
      },
    ],
    tags: [
      {
        name: "Funny",
      },
      {
        name: "Fiction",
      },
      {
        name: "Horror",
      },
    ],
  },
];

export default data;
