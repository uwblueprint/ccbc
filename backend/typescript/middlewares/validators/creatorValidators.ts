...
export const creatorDtoValidator = async (
  ...
  if (!validatePrimitive(req.body.street_address, "string")) {
    return res.status(400).send(getApiValidationError("street_address", "string"))
  }
  ...
  if (!validatePrimitive(req.body.profile_picture_link, "string")) {
    return res.status(400).send(getApiValidationError("profile_picture_link", "string"))
  }
  if (!validatePrimitive(req.body.availability, "string")) {
    return res.status(400).send(getApiValidationError("availability", "string"))
  }
  ...
  if (!validatePrimitive(req.body.isReadyForReview, "boolean")) {
    return res.status(400).send(getApiValidationError("isReadyForReview", "boolean"))
  }
